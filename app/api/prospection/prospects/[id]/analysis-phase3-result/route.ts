...
export async function POST(
  ...
  if (error) {
    console.error('Error saving phase 3 result:', error);
  ...
  await supabase
    .schema('agent_business_analyst')
    .from('missions')
    .update({ statut_phase_3: body.statut || 'completed' })
    .eq('id', missionId);
...
export async function GET(
  ...
  if (error) {
    ...
    console.error('Error fetching phase 3 result:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
